...
const AdminSidebar = () => {
  ...
  const baseStyle = 'flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-300 ease-in-out hover:scale-105 hover:bg-gradient-to-r hover:from-blue-500 hover:to-blue-600 hover:text-white';
  const activeStyle = 'bg-gradient-to-r from-blue-600 to-blue-500 text-white font-semibold shadow-lg';
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-screen w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 shadow-lg transform transition-transform duration-300 ease-in-out overflow-y-auto pt-14 md:pt-0 z-40 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static`}>
        ...
        <div className="p-6 text-center border-b border-gray-200 dark:border-gray-700">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage users & system</p>
        </div>

        {/* Navigation */}
        <nav className="flex flex-col text-sm text-gray-800 dark:text-gray-200 p-4 pb-10 space-y-2">
          <NavLink to="/admin/dashboard" onClick={closeSidebar} className={({ isActive }) => `${baseStyle} ${isActive ? activeStyle : ''}`}>
            <FaTachometerAlt /><span>Dashboard</span>
          </NavLink>
          <NavLink
            to="/admin/manage-temples" onClick={closeSidebar} className={({ isActive }) => `${baseStyle} ${isActive ? activeStyle : ''}`}>
          <FaUsers /><span>Manage Temples</span>
          </NavLink>
          <NavLink
            to="/admin/manage-users" onClick={closeSidebar} className={({ isActive }) => `${baseStyle} ${isActive ? activeStyle : ''}`}>
          <FaUsers /><span>Manage Users</span>
          </NavLink>
          <NavLink
            to="/admin/user-feedback" onClick={closeSidebar} className={({ isActive }) => `${baseStyle} ${isActive ? activeStyle : ''}`}>
          <FaUsers /><span>User Feedback</span>
          </NavLink>
